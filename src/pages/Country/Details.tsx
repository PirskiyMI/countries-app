...
export const DetailsInfo = (props: DetailsInfoProps) => {
   const [neighborsList, setNeighborsList] = useState([]);
   const { data } = countryApi.useFetchCountryNeighboursQuery(props.borders.join(','));
   useEffect(() => {
      if (data) {
   ...
   return (
      <div className={styles.country_info}>
         <h2 className={styles.country_title}>{props.name.common}</h2>
...
export const DetailsWrapper = ({ flags, ...etc }: ICountryCard) => {
   console.log(etc);
